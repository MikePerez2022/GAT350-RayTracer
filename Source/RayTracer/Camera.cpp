...
void Camera::CalculateViewPlane()
{
	...
	float height = halfHeight * 2.0f;// <twice the size of half height>;
	...
	m_lowerLeft = m_eye - (m_horizontal * 0.5f) - (m_vertical * 0.50f) - m_forward;
}
...
ray_t Camera::GetRay(const glm::vec2& point) const
{
	...
	ray.origin = m_eye;// <set ray origin>
